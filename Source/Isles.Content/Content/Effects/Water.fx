				  ...
				  out float2 oPosWs : TEXCOORD3)
{
	...
	oPosition = pos_ps;
	oUV = uv * WaveTextureScale + WaveTextureOffset;
	oPosWs = pos_ws.xyz;
...
{	
	...
	float3 N = normalize(tex2D(WaveSampler, uv).xyz);	
	...
	return tex2D(ReflectionSampler, rUV + N.xy * DistortionScale) * float4(lightResult.Diffuse, Alpha) + float4(lightResult.Specular.rgb, 0);
}
