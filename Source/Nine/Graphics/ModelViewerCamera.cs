...
namespace Nine.Graphics
{
    public class ModelViewerCamera : ICamera
    {
        ...
        public Matrix Projection 
        {
            get 
            {
                return Matrix.CreatePerspectiveFieldOfView(
                    MathHelper.PiOver4, GraphicsDevice.Viewport.AspectRatio, MinRadius, MaxRadius * 2);
            }
        }
        ...
        public ModelViewerCamera(IServiceProvider services, IInput input) : this(services, input, 20, 1, 100, Vector3.Up) { }

        public ModelViewerCamera(IServiceProvider services, float radius, float minRadius, float maxRadius, Vector3 up)
            : this(services, null, radius, minRadius, maxRadius, up)
        { }
        
        public ModelViewerCamera(IServiceProvider services, IInput input, float radius, float minRadius, float maxRadius, Vector3 up)
        {
        ...
        {
            if (Input is IUpdateObject)
            {
                IUpdateObject update = Input as IUpdateObject;
