namespace Nine
{
    ...
    class ContentBuilder : IDisposable
    {
        ...
        {
            "Microsoft.Xna.Framework.Content.Pipeline" + xnaVersion,
        ...
        private void CreateBuildProject()
        {
            ...
            buildProject.AddItem("Reference", typeof(Nine.Content.Pipeline.Processors.ExtendedModelProcessor).Assembly.FullName);
        ...
        /// </summary>
        public void Add(string filename, string name, string importer, string processor, IDictionary<string, object> processorParameters)
        {
            ProjectItem item = buildProject.AddItem("Compile", Path.GetFullPath(filename))[0];

            item.SetMetadataValue("Link", Path.GetFileName(filename));
            item.SetMetadataValue("Name", string.IsNullOrEmpty(name) ? Path.GetFileNameWithoutExtension(filename) : name);
            ...
            if (processorParameters != null)
            {
                ...
                {
                    item.SetMetadataValue(string.Format("ProcessorParameters_{0}", param.Key), ConvertToStrign(param.Value));
                }
            }
        ...
        private string ConvertToStrign(object value)
        {
        ...
        public void Build()
        {
            ...
            {
                throw new InvalidContentException(string.Join("\n", errorLogger.Errors.ToArray()));
            }
        }
        ...
        void CreateTempDirectory()
        {
            ...
            baseDirectory = Path.Combine(Environment.CurrentDirectory, GetType().FullName);
