                ...
                out float4 Color : COLOR )
{
    ...
    for(int i=0; i< numLights; i++)
    {
        ...
        {
            fade = max(1 - pow(max(distance / lights[i].Range, 0.000001), lights[i].Attenuation), 0);
            if (angle < inner)
                fade *= pow(max((angle - outer) / (inner - outer), 0.000001), lights[i].Falloff);
        }

        diffuse += lights[i].DiffuseColor * dotL * zeroL * fade;
        specular += lights[i].SpecularColor * pow(max(dotH, 0.000001) * zeroL, SpecularPower) * fade;
    }
