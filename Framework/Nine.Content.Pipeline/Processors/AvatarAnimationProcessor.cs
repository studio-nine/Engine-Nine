...
namespace Nine.Content.Pipeline.Processors
{
    ...
    public class AvatarAnimationProcessor : ContentProcessor<NodeContent, BoneAnimationClipContent>
    {
        ...
        private BoneAnimationClipContent ConvertData(AvatarAnimationData data)
        {
            ...
            clip.Transforms = Enumerable.Range(0, AvatarRenderer.BoneCount).Select(
                                    index => data.Keyframes.Where(f => f.Bone == index).Select(f => f.Transform).ToArray()).ToArray();
            return clip;
        }
        
        private AvatarAnimationData ProcessAnimationData(NodeContent input, ContentProcessorContext context)
        {
            // Find the skeleton
            ...
            AvatarAnimationData avatarCustomAnimationData = null;
                                                                  ...
                                                                  in skeleton.Animations)
            {
                ...
                avatarCustomAnimationData = new AvatarAnimationData(animation.Key,
