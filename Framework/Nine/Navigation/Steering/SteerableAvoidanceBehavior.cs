    ...
    public class SteerableAvoidanceBehavior : SteeringBehavior
    {
        ...
        protected override Vector2 OnUpdateSteeringForce(float elapsedTime, Steerable movingEntity)
        {
            ...
            if (nearestSteerable != null)
            {
                ...
                foreach (Steerable secondPartner in Neighbors.Find(new Vector3(movingEntity.Position, 0), secondDetectorLength))
                {
        ...
        {
            foreach (Steerable partner in Neighbors.Find(new Vector3(movingEntity.Target.Value, 0), movingEntity.BoundingRadius))
            {
                if (partner == null || partner == movingEntity)
                    continue;
