                ...
                out float4 Color : COLOR )
{
    ...
    for(int i=0; i< numLights; i++)
    {
        ...
        float fade = max(0, (pow(max(lights[i].Attenuation, 0.000001), 1 - distance / lights[i].Range) - 1) / (lights[i].Attenuation - 1));
    
        diffuse += lights[i].DiffuseColor * zeroL * dotL * fade;
        specular += lights[i].SpecularColor * pow(max(dotH, 0.00001) * zeroL, SpecularPower) * fade;
    }
