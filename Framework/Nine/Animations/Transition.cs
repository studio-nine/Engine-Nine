...
namespace Nine.Animations
{
    ...
    internal sealed class TransitionCallback<T> where T : struct
    {
        ...
        public TransitionCallback(object target, string member)
        {
            ...
            if (Member == null)
            {
                throw new ArgumentException("Object of type " + target.GetType().Name +
        ...
        public static bool operator ==(TransitionCallback<T> x, TransitionCallback<T> y)
        {
            return (x.Callback != null && x.Callback == y.Callback) ||
                   (x.Object != null && x.Object == y.Object &&

                   // FIX for Windows Phone 7
                   x.Member.Name == y.Member.Name &&
                   x.Member.DeclaringType == y.Member.DeclaringType);
        }
    ...
    {
        EnumerableCollection<TransitionChannel<T>> activeTransitions = new EnumerableCollection<TransitionChannel<T>>();
        EnumerableCollection<TransitionChannel<T>> delayedTransitions = new EnumerableCollection<TransitionChannel<T>>();
        ...
        public void Start(ITweener<T> tweener, TimeSpan delay, bool useFutureFromValue, TransitionCallback<T> callback)
        {
            ...
            if (tweener is IAnimation)
                (tweener as IAnimation).Completed += new EventHandler(Transition_Complete);
