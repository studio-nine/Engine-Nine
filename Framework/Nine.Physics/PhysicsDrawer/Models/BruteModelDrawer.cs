...
namespace BEPUphysicsDrawer.Models
{
    ...
    /// </summary>
    class BruteModelDrawer : ModelDrawer
    {
        ...
        public BruteModelDrawer(GraphicsDevice graphics)
            : base(graphics)
        {
        ...
        protected override void DrawManagedModels(Matrix viewMatrix, Matrix projectionMatrix)
        {
            ...
            for (int i = 0; i < effect.CurrentTechnique.Passes.Count; i++)
            {
                foreach (BruteDisplayObjectEntry entry in displayObjects)
                {
                    entry.Draw(textures, GraphicsDevice, effect, effect.CurrentTechnique.Passes[i]);
                }
            }
        }
    }
