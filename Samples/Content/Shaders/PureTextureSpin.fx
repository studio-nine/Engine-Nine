/*********************************************************************NVMH3****
File:  $Id: //sw/devtools/FXComposer1.6/SDK/MEDIA/HLSL/PureTextureSpin.fx#1 $
...
vertData pureTexVS(appdata IN, uniform float2 CS)
{
    vertData OUT;
    float nx = IN.Position.x - XOff;
	float4 objPos = float4(
		nx * CS.x - IN.Position.z * CS.y,
		IN.Position.y,
		nx * CS.y + IN.Position.z * CS.x,
		1.0);
...
> {		
		VertexShader = compile vs_1_1 pureTexVS(spinner());
		ZEnable = true;
		ZWriteEnable = true;
		CullMode = None;
