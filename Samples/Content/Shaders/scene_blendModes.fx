/*********************************************************************NVMH3****
File:  $Id: //sw/devtools/FXComposer1.6/SDK/MEDIA/HLSL/scene_blendModes.fx#1 $
...
Comments:
    Remapping textures into screen space for 2D image processing.
    These techniques emulate blend modes found in Photoshop(tm).
...
FILE_TEXTURE_2D(BaseImg,BaseImgSampler,"Veggie.dds")
FILE_TEXTURE_2D(BlendImg,BlendImgSampler,"Blended.dds")

/*
texture BaseImg : DIFFUSE
<
    string ResourceName = "Veggie.dds"; // "default_color.dds";
    string ResourceType = "2D";
>;

sampler2D BaseImgSampler = sampler_state
{
    Texture = <BaseImg>;
    MinFilter = Linear;
    MagFilter = Linear;
...
texture BlendImg : DIFFUSE
<
    string ResourceName = "Blended.dds";
    string ResourceType = "2D";
>;

sampler2D BlendImgSampler = sampler_state
{
    Texture = <BlendImg>;
    MinFilter = Linear;
    MagFilter = Linear;
...
#define GET_PIX float4 base = tex2D(BaseImgSampler, IN.UV); \
...
#define TECH(name,psName) technique name <string Script="Pass=p0;";> { \
...
#define TECH2(name,psName) technique name <string Script="Pass=p0;";> { \
    pass p0 <string Script="Draw=Buffer;";> { \
